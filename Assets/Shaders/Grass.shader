Shader "Custom/Grass"
{
    Properties
    {
		...
		[Header(Bending)]
		_YOffset("Y offset", float) = 0.0// Y offset, no animation below 0 value  
	...
	#include "./Includes/CustomTessellation.cginc"
	...
	void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
	{
		...
		if(shouldCreateGrass==3){
			...
			// Radially bend grass away from xyz coordinates
			...
			for  (int x = 0; x < _PositionArray; x++){
				...
				sphereDisp *= radius; // Falloff: Position multiplied by radius
			...
			for (int i = 0; i < BLADE_SEGMENTS; i++)
			{
				...
				float segmentWidth = i == 0 ? width/4 : width * (1 - t); // Slim down bottom segments
